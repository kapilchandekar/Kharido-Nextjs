...
import { otpValidation } from "@/utils/validation/Validation";
import { useOtpMutation } from "@/lib/userSlice/userSlice";
...
const verifyOtp = () => {
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values, { resetForm }) => {
      try {
        const response = await otp(values).unwrap();

        if (response) {
          // Navigate to home page on successful login
          toast.success(response.message);
          // router.push("/resetpassword");
          router.push(`/resetpassword/${response.userId}`);
          resetForm();
        }
        ...
        <form className="space-y-6" onSubmit={formik.handleSubmit}>
          <div>
            ...
            <div className="mt-2">
              ...
              />
              <span className="text-pink-700">{formik.errors.otp}</span>
            </div>
          </div>
          <div>
            <button
